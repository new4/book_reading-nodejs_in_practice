...
正在使用 `EventEmitter` 且在异常发生的时候希望能够优雅地处理，但它不断抛出异常？

任何从 `EventEmitter` 继承的自定义类或者标准类都可以在 `error` 事件上添加 一个监听器来处理。

在 `Node` 中，`error` 事件被当做特殊情况，假如没有监听器，那么默认的动作是打印一个堆栈并退出程序。
